...
Original file is located at
    https://colab.research.google.com/drive/1coIBpZ5oCTN4lkDMsYqKezQsUg38911Z

# **Packages Installation**
"""

# Install required packages
!pip install torch torchvision pillow numpy matplotlib scikit-learn opencv-python
...
# Configuration
CONFIG = {
    'data_dir': '/cedar_dataset',
    'genuine_dir': '/cedar_dataset/full_org',
    'forged_dir': '/cedar_dataset/full_forg',
    'output_dir': '/cedar_dataset/processed_cedar',
    'output_dir': '/cedar_dataset/cedar_splits',
...
"""# **Visualization - Original vs Preprocessed**"""

def visualize_preprocessing(genuine_dir, forged_dir, img_size):
    """Visualize original vs preprocessed images"""

    fig, axes = plt.subplots(2, 4, figsize=(14, 6))
    fig.suptitle('Original vs Preprocessed Signatures', fontsize=14)

    # Get sample files
    genuine_file = os.path.join(genuine_dir, os.listdir(genuine_dir)[0])
    forged_file = os.path.join(forged_dir, os.listdir(forged_dir)[0])

    samples = [
        (genuine_file, 'Genuine'),
        (forged_file, 'Forged')
    ]

    for row, (img_path, label) in enumerate(samples):
        # Original
        original = load_image_raw(img_path)
        ...
        axes[row, 1].set_title('After Blur')
        axes[row, 1].axis('off')

        # After Thresholding
        _, thresh = cv2.threshold(blurred, 0, 255, cv2.THRESH_BINARY + cv2.THRESH_OTSU)
        axes[row, 2].imshow(thresh, cmap='gray')
        axes[row, 2].set_title('After Threshold')
        axes[row, 2].axis('off')

        # Final Preprocessed
        preprocessed = preprocess_signature(img_path, img_size)
        axes[row, 3].imshow(preprocessed, cmap='gray')
        axes[row, 3].set_title(f'Final ({img_size[1]}×{img_size[0]})')
        axes[row, 3].axis('off')

    plt.tight_layout()
    plt.show()

visualize_preprocessing(CONFIG['genuine_dir'], CONFIG['forged_dir'], CONFIG['img_size'])

"""# **Visualization - Training Batch**"""
...
def visualize_batch(dataloader, num_samples=8):
    ...
    fig.suptitle('Training Batch Samples', fontsize=14)
    ...
    for i in range(num_samples):
        ...
        label = labels[i].item()
        label_text = 'Genuine' if label == 1 else 'Forged'
        color = 'green' if label == 1 else 'red'

        axes[row, col].imshow(img, cmap='gray')
        axes[row, col].set_title(label_text, color=color)
...
"""# **Save Splits to Pickles**


"""

def save_splits(splits, output_dir):
    """Save splits dictionary to pickle file"""

    # Use /content/ directory (not inside cedar_dataset)
    output_path = '/cedar_dataset/cedar_splits'
    ...
    # Save splits pickle
    splits_file = os.path.join(output_path, 'splits.pkl')
    ...
    print(f"  - val:   {len(splits['val'])} samples")
    print(f"  - test:  {len(splits['test'])} samples")
    print(f"  - writer_assignments: train/val/test writer IDs")

    return splits_file

# Save splits
splits_file = save_splits(splits, CONFIG['output_dir'])

"""# **Download Processed Data**"""

def prepare_download(splits, output_dir, img_size):
    """
    Prepare processed dataset for download
    ...
    print("Preparing processed dataset for download...")

    # Create directories
    genuine_out = os.path.join(output_dir, 'genuine')
    forged_out = os.path.join(output_dir, 'forged')
    ...
    for i, (src_path, label) in enumerate(all_files):
        if (i + 1) % 500 == 0:
            print(f"Processing: {i + 1}/{total}")

        # Preprocess image
        img = preprocess_signature(src_path, img_size)

        # Determine output path
        filename = os.path.basename(src_path)
        if label == 1:
            dst_path = os.path.join(genuine_out, filename)
        else:
            dst_path = os.path.join(forged_out, filename)

        # Save preprocessed image
        img.save(dst_path)

    # Save splits with relative paths
    relative_splits = {
        'train': [(os.path.basename(p), l) for p, l in splits['train']],
        'val': [(os.path.basename(p), l) for p, l in splits['val']],
        'test': [(os.path.basename(p), l) for p, l in splits['test']],
        'writer_assignments': splits['writer_assignments']
    }

    with open(os.path.join(output_dir, 'splits.pkl'), 'wb') as f:
        pickle.dump(relative_splits, f)
    ...
    # Create ZIP file
    zip_path = '/cedar_dataset/cedar_processed.zip'
    print(f"\nCreating ZIP file...")
...
# download_path = prepare_download(splits, CONFIG['output_dir'], CONFIG['img_size'])
print("To download processed data, uncomment and run:")
print("download_path = prepare_download(splits, CONFIG['output_dir'], CONFIG['img_size'])")